import { Button } from "@renderer/components/ui/button"
import { useT } from "@renderer/i18n"
import { Info } from "phosphor-react"

interface WelcomeStepProps {
	onNext: () => void
}

export function WelcomeStep({ onNext }: WelcomeStepProps): JSX.Element {
	const t = useT()
	return (
		<div className="space-y-4">
			<h2 className="type-hero text-strong">{t("Set up Rich Presence")}</h2>
			<p className="type-body text-muted-foreground">
				{t("Two steps, and Discord shows what you play in VLC.")}
			</p>

			<div className="flex gap-3 rounded-md border border-divider bg-inset p-4">
				<Info aria-hidden="true" className="mt-[2px] size-4 shrink-0 text-brand" />
				<div className="space-y-2 type-body text-body">
					<p>
						{t(
							"The app reads VLC's HTTP interface to see what is playing. The next step turns that interface on.",
						)}
					</p>
					<p>
						{t(
							"If a file has embedded cover art, the app uploads that image to public hosts so Discord can display it. Anyone with the link can view it while the host keeps it.",
						)}
					</p>
				</div>
			</div>

			<div className="flex justify-end gap-2 pt-4">
				<Button onClick={onNext}>{t("Get started")}</Button>
			</div>
		</div>
	)
}
