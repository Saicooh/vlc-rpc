...
describe("parse, western naming, no group tag", () => {
	it("drops a directory path before parsing a VLC filename", () => {
		const result = parse("C:\\Videos\\The.Matrix.1999.1080p.BluRay.x264.mp4")
	...
	it("recovers a multi-word episode subtitle after the S/E marker", () => {
		const result = parse("Some.Show.S01E03.The Long Night.1080p.WEB-DL.mkv")
		expect(result.subtitle).toBe("The Long Night")
	})

	it("keeps the arc subtitle and removes a franchise year from an underscored release", () => {
		const result = parse("JoJos_Bizarre_Adventure_2012_S06E01_STEEL_BALL_RUN_1080p_NF_WEB.mkv")

		expect(result.title).toBe("JoJos Bizarre Adventure")
		expect(result.subtitle).toBe("Steel Ball Run")
		expect(result.year).toBe(2012)
		expect(result.season).toBe(6)
		expect(result.episode).toBe(1)
	})

	it("recognizes a zero-padded episode in a short release without S/E markers", () => {
		const result = parse(
			"Puella Magi Madoka★Magica - BDRIP - 03 1920x1080 Independent proyects",
			24 * 60 + 11,
		)

		expect(result.title).toBe("Puella Magi Madoka★Magica")
		expect(result.episode).toBe(3)
	})

	it("recovers an absolute episode when a grouped release ends in a filename extension", () => {
		const result = parse("[SR] JoJo's Bizarre Adventure Stardust Crusaders - 37.mkv")

		expect(result.title).toBe("JoJo's Bizarre Adventure Stardust Crusaders")
		expect(result.episode).toBe(37)
		expect(result.signal).toBe("fansub")
	})

	it("removes a trailing movie descriptor from an anime release", () => {
		const result = parse(
			"[Erai-raws] Chainsaw Man - Reze-hen - Movie [1080p CR WEBRip HEVC AAC][MultiSub][EB732A7B]",
		)

		expect(result.title).toBe("Chainsaw Man - Reze-hen")
	})

	it("removes a trailing special descriptor from an anime release", () => {
		const result = parse(
			"[Erai-raws] Fate Strange Fake - Whispers of Dawn - SP [1080p][HEVC][Multiple Subtitle][65A9F263]",
		)

		expect(result.title).toBe("Fate Strange Fake - Whispers of Dawn")
	})
