...
export class Client {
	...
	private statusTimeoutMs(): number {
		...
		const raw =
			typeof configured === "number" && configured > 0 && configured <= 10
				? configured * 1000
				: configured
	...
	public async readStatus(forceUpdate = false): Promise<VlcStatus | null> {
		...
		try {
			...
			if (reason === "timeout") {
				...
				// VLC is not running or HTTP interface is not listening.
				// This is expected when VLC is closed, avoid spamming logs.
