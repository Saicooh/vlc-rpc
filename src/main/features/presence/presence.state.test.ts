...
describe("Custom Discord button", () => {
	it.each(["playing", "paused"] as const)("adds the configured button while %s", async (state) => {
		config.current = {
			...BASE_CONFIG,
			customButtonEnabled: true,
			customButtonLabel: " My Profile ",
			customButtonUrl: " https://example.test/profile ",
		}
		const service = videoService(null, {
			resolve: async () => ({
				imageUrl: "https://example.test/anime.jpg",
		...
		const presence = await service.getDiscordPresence(
			videoStatus("Some.Anime.S01E01.mkv", state),
			timeline,
		)

		expect(presence?.buttons).toEqual([
			{ label: "View on AniList", url: "https://anilist.co/anime/1" },
			{ label: "My Profile", url: "https://example.test/profile" },
		])
	})

	it("does not send a button when it is enabled without a URL", async () => {
		config.current = {
			...BASE_CONFIG,
			customButtonEnabled: true,
			customButtonUrl: "   ",
		...
		const presence = await service.getDiscordPresence(
			videoStatus("Some.Anime.S01E01.mkv"),
			timeline,
		)
...
describe.each([{ state: "playing" as const }, { state: "paused" as const }])(
	...
	({ state }) => {
		...
		it("reads a zero-padded episode from a short release filename", async () => {
			const filename = "Puella Magi Madoka★Magica - BDRIP - 03 1920x1080 Independent proyects"
			const status = videoStatus(filename, state)
			status.playback.duration = 24 * 60 + 11
			status.media.filename = filename

			const presence = await videoService(null).getDiscordPresence(status, timeline)

			expect(presence?.details).toBe("Puella Magi Madoka★Magica")
			expect(presence?.state).toBe("Episode 3")
		})

		it("uses the cover resolver's canonical title for a western series", async () => {
			const presence = await videoService(null, {
				resolve: async () => ({
				...
				videoStatus("better.call.saul.s06e06.1080p.web.h264-cakes", state),
				timeline,
			)

			expect(presence?.details).toBe("Better Call Saul")
			expect(presence?.large_image).toBe("https://static.tvmaze.com/better-call-saul.jpg")
		})
...
describe("Presence small image", () => {
	it("shows Syncplay and keeps the AniList source and custom buttons", async () => {
		config.current = {
			...BASE_CONFIG,
			customButtonEnabled: true,
			customButtonLabel: "Profile",
			customButtonUrl: "https://example.test/profile",
		}
		const cover: VideoCoverResult = {
		...
		).getDiscordPresence(videoStatus("The.Matrix.1999.mkv"), timeline)

		expect(presence?.small_text).toBe("Watching Together")
		expect(presence?.buttons).toEqual([
			{ label: "View on AniList", url: "https://anilist.co/anime/1" },
			{ label: "Profile", url: "https://example.test/profile" },
		])
	})

	it("does not add the custom button to a non-anime video", async () => {
		config.current = {
			...BASE_CONFIG,
			customButtonEnabled: true,
			customButtonLabel: "Profile",
		...
		const presence = await videoService(null, { resolve: async () => cover }).getDiscordPresence(
			videoStatus("better.call.saul.s06e06.1080p.web.h264-cakes"),
			timeline,
		)

		expect(presence?.buttons).toEqual([
			{ label: "View on IMDB", url: "https://www.imdb.com/title/tt0133093/" },
		])
	})
